...
module Guts
  class SessionsControllerTest < ActionController::TestCase
    ...
    test 'should show error for invalid user' do
      post :create, params: {
        session: { email: 'durkadurka@gmail.com' }
      }

      assert_response :success
      assert_equal 'Invalid login credentials', flash[:alert]
    end
    ...
    test 'should not set a forgot password token for invalid user' do
      ...
      assert_equal 'Invalid email address', flash[:alert]
    end

    test 'should reset the users password' do
      get :reset_password, params: { token: 123456 }
